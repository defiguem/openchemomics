function [sadecomp_res] = sadecomp(X, Y, adecomp_res)
%
...
Xf = cell(1,length(adecomp_res.Xf));
Ef = cell(1,length(adecomp_res.Xf));
ssq = zeros(1,neff);
ssqvarexp = zeros(1,neff);

% Retrieves options used for adecomp
Options = adecomp_res.Options;
    ...
    case 'classical'
        ...
        for i = 1 : neff
            ...
            for j = 1 : size(Ysm{1,i}{1},1)
                ...
                Ysm{1,i}{2}(j,:) = mean(adecomp_res.Xf{i}(idx == 1, :),1);
        ...
        for i = 1 : neff + 1 % +1 for the grand mean
            ...
            if i == 1 % grand mean
                ...
                Xm = repmat(adecomp_res.Xm(1,:),[size(Y,1),1]); 

            else % effects

                Xf{i-1} = zeros(size(X));
                for k = 1 : size(Ysm{i-1}{1},1)
                    idx = ismember(Y(:, unique(adecomp_res.effects{i-1})), Ysm{i-1}{1}(k,:),'rows');
    ...
    case {'glm','rebalanced'}
        ...
        for i = 1 : length(cod) % all main effects and interactions
            ...
            if i == 1 % grand mean matrix
                ...
                Ef{1,i-1} = X - (cat(2,Xftmp{:}) * cat(2,Bf{:})');
        ...
        for i = 1 : neff + 1
            ...
            if i < neff + 1 % effects
                ...
                ssqvarexp(1,i) = ssq(1,i) / ( sum((X-Xm).^2,'all') ) * 100;
                ...
                % Explained variance (row 1 does not sum to 100%, row 2 sums to 100%)
                ssqvarexp(1,i) = sum(Xe(:).^2) / ( sum((X-Xm).^2,'all') ) * 100;
...
sadecomp_res.X = X; % experimental data
...
sadecomp_res.effects = adecomp_res.effects; % effects
